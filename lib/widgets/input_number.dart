    ...
    return TextFormField(
      ...
      inputFormatters: <TextInputFormatter>[
        ...
        TextInputFormatter.withFunction((
          ...
          debugPrint(
            "Operator detected (n: $opCount) - old: ${oldValue.text}, new: ${newValue.text}",
          );

          if (opCount > 1) {
            // Only when last operator was inserted at end
            if (newValue.text.startsWith(oldValue.text)) {
              debugPrint("do evaluation");
              final Expression? exp = Expression.tryParse(oldValue.text);
              if (exp != null) {
                final double result =
                    const ExpressionEvaluator()
                        ...
                        .toDouble() ??
                    0.0;
                debugPrint(newValue.selection.start.toString());
